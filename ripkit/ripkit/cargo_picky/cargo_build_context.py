...
def get_cargo_version():
    try:
        rust_version = subprocess.check_output(
            ["cargo", "--version"], stderr=subprocess.STDOUT, text=True
        )
...
def get_rust_version():
    try:
        rust_version = subprocess.check_output(
            ["rustc", "--version"], stderr=subprocess.STDOUT, text=True
        )
        return rust_version.strip()
    except subprocess.CalledProcessError as e:
        return f"Error: {e.output.strip()}"
...
def get_cargo_metadata(manifest_path=Path("Cargo.toml")):
    try:
        ...
        cargo_metadata_output = subprocess.check_output(
            cargo_metadata_command, stderr=subprocess.STDOUT, text=True
        )
