...
def del_crate(crate: Union[list[str], str], dir=CLONED_DIR) -> None:
...
def clone_crate(crate: Union[list[str], str], exist_ok=False, dir=CLONED_DIR) -> None:
    """
...
def is_executable(path: Path) -> bool:
    try:
        ...
        if not hasattr(bin, "format"):
            return False
        elif bin.format in [lief.EXE_FORMATS.PE, lief.EXE_FORMATS.ELF]:
            return True
        return False
    except Exception as e:
...
def get_build_productions(
    ...
    */target/release .
    ...
    for path in target_dir.iterdir():
        ...
        if path in [target_dir.joinpath("debug"), target_dir.joinpath("target")]:
            continue
