...
def save_and_register_analysis(
    bin_path: Path,
    analysis_data: Union[
    ...
    compiler: Compiler,
    fileType: FileType,
    opt_lvl: Union[RustcOptimization, GoOptimization, Coptimization],
    os: str = "",
    arch: str = "",
    stripped_level=None,
...
def ___re_analyze_tmp(path, comp, lang, opt):
    ...
    bin_name = [
        x.parent.joinpath(x.name.replace("_ANALYSIS.npz", "")) for x in analyzed_files
    ]
    ...
    for file in alive_it(bin_files):
        ...
        data_gen = generate_minimal_labeled_features(file, use_one_hot=False)

        save_and_register_analysis(file, data_gen, analysis_type, lang, comp, ft, opt)

        analysis_type = AnalysisType.ONEHOT_PLUS_FUNC_LABELS
        data_gen = generate_minimal_labeled_features(file, use_one_hot=True)
...
def _update_pkg_in_reg(old_pkg_path, new_pkg_name):
    ...
    files_in_pkg = reg[reg["package_path"] == f"{old_pkg_path.resolve()}"]
    ...
    # analysis path
    files_in_pkg.loc[
        files_in_pkg["package_path"] == f"{old_pkg_path.resolve()}", "analysis_path"
    ] = files_in_pkg.loc[
        files_in_pkg["package_path"] == f"{old_pkg_path.resolve()}", "analysis_path"
    ].str.replace(
        f"{old_pkg_path.resolve()}", f"{new_path.resolve()}"
    )

    # package path
    files_in_pkg.loc[
        files_in_pkg["package_path"] == f"{old_pkg_path.resolve()}", "package_path"
    ] = files_in_pkg.loc[
        files_in_pkg["package_path"] == f"{old_pkg_path.resolve()}", "package_path"
    ].str.replace(
        f"{old_pkg_path.resolve()}", f"{new_path.resolve()}"
    ...
    # Make sure the old_pkg_path is in the registry
    should_be_empty = reg[reg["package_path"] == f"{old_pkg_path.resolve()}"]
