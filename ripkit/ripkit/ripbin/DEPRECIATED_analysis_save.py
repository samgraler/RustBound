...
def write_generator_to_parquet(generator, output_path):
    ...
    )  # Adjust the schema according to your numpy array structure
...
def db_save_analysis(
    binary_path: Path,
    data: Union[pd.DataFrame, np.ndarray, Generator[np.ndarray, None, None]],
    progLang: ProgLang,
    compiler: Compiler,
    fileType: FileType,
    opt_lvl: Union[RustcOptimization, GoOptimization],
    is_stripped,
    ...
    descriptive_name = "_".join(
        [bin_toplvl_name, opt_lvl.value, "STRIPPED" if is_stripped else "NOTSTRIPPED"]
    )
